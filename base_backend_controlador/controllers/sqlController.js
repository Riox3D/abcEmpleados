...
export const proxyToSqlAuth = async (req, res) => {
  try {
    console.log('req.user.numEmp',req.user)
    const endpoint = `${req.originalUrl}/${req.user.id}/${req.user.numEmp}` // quita /sql/
    const method = req.method.toLowerCase() // post, get, put, delete...
    const data = {
      body: req.body,
      query: req.query,
...
export const proxyToSqlAuthRol = async (req, res) => {
  try {
    console.log('req.user.numEmp',req.user)
    const endpoint = `${req.originalUrl}/${req.user.id}/${req.user.numEmp}/${req.user.role}` // quita /sql/
    const method = req.method.toLowerCase() // post, get, put, delete...
    const data = {
      body: req.body,
      query: req.query,
