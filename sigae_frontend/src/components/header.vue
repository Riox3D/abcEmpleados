    ...
    <q-toolbar class="q-px-md" style="min-height: 70px; position: relative">
      <div class="row items-center justify-between full-width">
        ...
        <div class="row items-center" style="min-width: 200px">
          <q-btn v-if="isLoggedIn" flat round icon="menu" color="white" class="q-mr-sm"
        ...
        <div style="position: absolute; left: 50%; transform: translateX(-50%)">
          <div class="text-h6 text-center">
            SIstema de Administración de Permisos e Incidencias
          </div>
        </div>
            ...
            <q-btn flat round dense color="white" class="q-mr-sm">
              ...
              <q-menu anchor="bottom right" self="top right">
                <q-list style="min-width: 250px; max-height: 300px" separator>
                  <q-item v-for="(notif, index) in notifications" :key="index" @click="handleSeleccionar(notif)"
                    clickable>
                    <q-item-section>
                      <q-item-label>Solicitud de {{ notif.nombre }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  <q-item v-if="notifications.length === 0">
                    <q-item-section>No hay notificaciones nuevas</q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
              ...
              <q-menu class="no-shadow bg-grey-8 text-white">
                <div class="row q-pa-md flex justify-center">
                  <div class="column items-center">
                    ...
                    <div class="flex justify-center text-subtitle1 q-mt-md q-mb-xs">
                      {{ auth.user.name }}
                    </div>
                    <div class="flex justify-center text-subtitle2">
                      {{ auth.user.direccion }}
                    </div>
                    <div class="flex justify-center text-body2">
                      {{ auth.user.gerencia }}
                    </div>
                    <div class="flex justify-center text-caption">
                      {{
                        auth.user.role == "aprobadorArea"
                          ? "APROBADOR"
                          : auth.user.role == "rh"
                            ? "RECURSOS HUMANOS"
                    ...
                    <q-btn class="q-mt-md q-mb-none q-pb-none full-width" color="grey-9" label="Cerrar Sesión"
                      unelevated size="sm" @click="logout" />
                  </div>
                </div>
              </q-menu>
            </q-avatar>
